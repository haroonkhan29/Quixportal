...
const Expenseformview = () => {
  ...
  const fetchAllProgress = async () => {
    try {
      const response = await axios.get("http://65.0.12.194:3000/expenseform");
...
const DataTable = ({ progressData, onSaveSuccess, onDelete, header, editedData, setEditedData }) => {
  ...
  const handleSave = async (index) => {
    if (progressData[index]) {
      ...
      try {
        await axios.put(`http://65.0.12.194:3000/expenseform/${progressId}`, editedData[index]);
        onSaveSuccess();
      } catch (error) {
  ...
  const handleDelete = async (progressId) => {
    try {
        await axios.delete(`http://65.0.12.194:3000/expenseform/${progressId}`);
        onSaveSuccess();
        onDelete(progressId); 
    } catch (error) {
