...
const Expenseformview = () => {
  ...
  const fetchAllProgress = async () => {
    try {
      const response = await axios.get("http://18.205.246.70:3001/expenseform");
...
const DataTable = ({ progressData, onSaveSuccess, onDelete, header, editedData, setEditedData }) => {
  ...
  const handleSave = async (index) => {
    if (progressData[index]) {
      ...
      try {
        await axios.put(`http://18.205.246.70:3001/expenseform/${progressId}`, editedData[index]);
        onSaveSuccess();
      } catch (error) {
  ...
  const handleDelete = async (progressId) => {
    try {
        await axios.delete(`http://18.205.246.70:3001/expenseform/${progressId}`);
        onSaveSuccess();
        onDelete(progressId); 
    } catch (error) {
