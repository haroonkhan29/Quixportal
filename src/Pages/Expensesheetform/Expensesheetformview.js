...
const Expensesheetformview = () => {
  ...
  const fetchAllProgress = async () => {
    try {
      const response = await axios.get("http://18.205.246.70:3001/expensesheet");
  ...
  const [localEditedData, setLocalEditedData] = useState({});
    ...
    const handleSave = async (index) => {
      if (progressData[index]) {
        ...
        try {
          await axios.put(`http://18.205.246.70:3001/expensesheet/${progressId}`, editedData[index]);
          onSaveSuccess();
        } catch (error) {
    ...
    const handleDelete = async (progressId) => {
      try {
          await axios.delete(`http://18.205.246.70:3001/expensesheet/${progressId}`);
          onSaveSuccess();
