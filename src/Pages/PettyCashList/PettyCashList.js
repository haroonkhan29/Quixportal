...
const PettyCashList = () => {
  ...
  const handleUpdateClick = async (id) => {
    ...
    try {
      ...
      const response = await fetch(`http://65.0.12.194:3000/pettycash/${id}`, {
        method: 'PUT',
        body: formData,
      });
      if (response.ok) {
              ...
              <tr key={item._id}>
                ...
                <td>
                  {editableRow === item._id ? (
                    ...
                    item.image && (
                      <img
                        src={`http://65.0.12.194:3000/${item.image}`}
                        alt="Expense Receipt"
                        style={{ maxWidth: '100px' }}
                        onClick={() => openFullScreenImage(`http://65.0.12.194:3000/${item.image}`)}
                      />
                    )
                  )}
                </td>
                ...
                <td className='button-container'>
                <button onClick={() => openFullScreenImage(`http://65.0.12.194:3000/${item.image}`)}>View</button>
                </td>
              </tr>
            ))}
        </tbody>
