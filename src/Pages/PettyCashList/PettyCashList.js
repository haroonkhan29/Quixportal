...
const PettyCashList = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://18.205.246.70:3001/pettycash');
  ...
  const handleUpdateClick = async (id) => {
    ...
    try {
      ...
      const response = await fetch(`http://18.205.246.70:3001/pettycash/${id}`, {
        method: 'PUT',
        body: formData,
      });
      if (response.ok) {
              ...
              <tr key={item._id}>
                ...
                <td>
                  {editableRow === item._id ? (
                    ...
                    item.image && (
                      <img
                        src={`http://18.205.246.70:3001/${item.image}`}
                        alt="Expense Receipt"
                        style={{ maxWidth: '100px' }}
                        onClick={() => openFullScreenImage(`http://18.205.246.70:3001/${item.image}`)}
                      />
                    )
                  )}
                </td>
                ...
                <td className='button-container'>
                <button onClick={() => openFullScreenImage(`http://18.205.246.70:3001/${item.image}`)}>View</button>
                </td>
              </tr>
            ))}
        </tbody>
