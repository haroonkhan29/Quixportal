...
const Admobview = () => {
  ...
  const handleView = (data) => {
    ...
    const images = admobData.map((item) => ({
      id: item._id,
      imageSrc: `http://65.0.12.194:3000/uploads/${item.profilePic}`,
      date: item.date,
  ...
  const handleDelete = async (index, id) => {
    try {
      await axios.delete(`http://65.0.12.194:3000/admob/${id}`);
  ...
  const fetchData = async (formattedDate) => {
    try {
      const apiUrl = "http://65.0.12.194:3000/admob";
          ...
          {selectedData && (
            <div className="selected">
              ...
              {selectedData.profilePic && (
                <img
                  src={`http://65.0.12.194:3000/uploads/${selectedData.profilePic}`}
                  alt="Selected"
                        ...
                        <td>
                          {data.profilePic && (
                            <img
                              ...
                              onClick={() =>
                                openImageModal(
                                  `http://65.0.12.194:3000/uploads/${data.profilePic}`
                                )
                              }
                            />
                          )}
