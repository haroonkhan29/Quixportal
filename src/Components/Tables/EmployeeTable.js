...
const EmployeeTable = () => {
  ...
  // const [formattedSelectedDate, setFormattedSelectedDate] = useState(formatDate(new Date())); // Removed
  ...
  const [editedAttendance, setEditedAttendance] = useState(null);
    ...
    const fetchAttendanceData = async () => {
      try {
        const response = await fetch(`http://18.205.246.70:3001/attendance?date=${selectedDate}`);
  ...
  return (
    <div>
     <div className="date-header">
      ...
      {!loading && !error && (
        <div>
        {/* <h2>{formattedSelectedDate}</h2> */} {/* Removed */}
          ...
          <tbody>
            {attendanceData.map((attendance, index) => (
              <tr key={index}>
                <td>
                  <img
                    src={`http://18.205.246.70:3001/uploads/${attendance.profilePic}`}
                    alt=""
